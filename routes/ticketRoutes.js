...
async function getData(Model) {
  return await Model.aggregate([
    // 1️⃣ Sirf "Working" wale record lo
    { $match: { status: 'Working' } },

    // 2️⃣ assign_to ka user ka data join karo
    {
      $lookup: {
        from: 'User', // collection ka naam
    ...
    // 3️⃣ Sirf wo record rakhna jisme kam se kam ek assigned Admin ho
    {
      $match: {
        'assigned_users.role': 'Admin'
      }
    },

    // 4️⃣ Sirf assigned Admin filter karke nikalna
    {
      $addFields: {
        assigned_admins: {
          $filter: {
